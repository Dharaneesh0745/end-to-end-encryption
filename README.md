# 🎯 System Architecture & Data Flow Diagrams
![Project Overview](public/image.png)

## 1️⃣ Overall System Architecture

```
┌─────────────────────────────────────────────────────────────────────┐
│                    SECURE MESSAGING SYSTEM                          │
└─────────────────────────────────────────────────────────────────────┘

Terminal 1 (Server)              TCP Socket              Terminal 2 (Client)
┌──────────────────┐          Port 9001              ┌──────────────────┐
│                  │◄───────────────────────────────►│                  │
│  server.exe      │         Encrypted Channel      │  client.exe      │
│                  │         (256-bit XOR)          │                  │
│  • Listens       │                                │  • Connects      │
│  • Accepts       │                                │  • Sends msgs    │
│  • Receives msgs │                                │  • Receives msgs │
│  • Encrypts      │                                │  • Encrypts      │
│  • Logs data     │                                │  • Logs data     │
│                  │                                │                  │
└──────────────────┘                                └──────────────────┘
         │                                                   │
         └───────────────────┬──────────────────────────────┘
                             │
                    ┌────────▼─────────┐
                    │  logs/messages   │
                    │     .txt file    │
                    │  (5 columns)     │
                    └──────────────────┘
```

---

## 2️⃣ Connection Handshake Flow

```
┌──────────────────────────────────────────────────────────────────────┐
│                    CONNECTION INITIALIZATION                         │
└──────────────────────────────────────────────────────────────────────┘

CLIENT                          SERVER
  │                               │
  │  1. Generate keypair         │
  │     • pk_c (32 bytes)        │
  │     • sk_c (32 bytes)        │
  │                               │
  │  2. Connect to port 9001      │
  ├──────────────────────────────►│
  │   (TCP SYN)                   │ 3. Accept connection
  │                               │ 4. Generate keypair
  │                               │    • pk_s (32 bytes)
  │                               │    • sk_s (32 bytes)
  │                               │
  │  5. Receive pk_s              │
  │◄──────────────────────────────┤ 6. Send pk_s (32 bytes)
  │   (exchange public keys)      │
  │                               │
  │ 7. Send pk_c (32 bytes)       │
  ├──────────────────────────────►│ 8. Receive pk_c
  │                               │
  │ ✅ Ready to exchange          │ ✅ Ready to exchange
  │    messages                   │    messages
  │ (Client knows: pk_c, sk_c,    │ (Server knows: pk_s, sk_s,
  │               pk_s)           │               pk_c)
  │                               │
```

---

## 3️⃣ Message Encryption Flow (Sender)

```
┌──────────────────────────────────────────────────────────────────────┐
│                    MESSAGE ENCRYPTION PROCESS                        │
└──────────────────────────────────────────────────────────────────────┘

User Input
    │
    ▼
"Hello Bob!"  ◄─── Maximum 1024 bytes
    │
    ├─────────────────────────────────────────┐
    │                                         │
    ▼                                         ▼
STEP 1: Generate Nonce           STEP 2: Derive Shared Key
    │                                │
    └──► random_bytes(16)      pk_recipient XOR sk_sender
         ↓                      ↓
    Nonce = [16 random bytes]   shared_key = 32 bytes
                                
                                ▼
                            STEP 3: Encrypt
                                │
                                └──► simple_encrypt()
                                    plaintext XOR shared_key
                                    (with state chaining)
                                    ↓
                                Ciphertext = 
                                [encrypted message]

                                ▼
                            STEP 4: Generate MAC
                                │
                                └──► compute_auth()
                                    HASH(ciphertext, sk_sender)
                                    ↓
                                MAC = [16-byte auth code]

                                ▼
                            STEP 5: Package Message
                                │
                                └──► [Nonce | Ciphertext | MAC]
                                    16B   +   (len+16)B   + 16B

                                ▼
                            STEP 6: Send Over Socket
                                │
                                └──► send(socket, packet)

                                ▼
                            STEP 7: Log to File
                                │
                                └──► [Time] | [Sender] | [Original]
                                     | [Hex] | [Details] | [N/A]
```

---

## 4️⃣ Message Decryption Flow (Receiver)

```
┌──────────────────────────────────────────────────────────────────────┐
│                    MESSAGE DECRYPTION PROCESS                        │
└──────────────────────────────────────────────────────────────────────┘

Socket Receive
    │
    ▼
[Nonce | Ciphertext | MAC] ◄─── From sender
    │
    ├──────┬──────────┬──────┐
    │      │          │      │
    ▼      ▼          ▼      ▼
   N    C(msg)      MAC   [reserved]

STEP 1: Verify MAC
    │
    └──► verify_auth()
        HASH(ciphertext, sk_sender) == received_MAC
        ↓
        ✓ PASS ──► Continue
        ✗ FAIL ──► Discard message

STEP 2: Derive Shared Key
    │
    └──► pk_sender XOR sk_recipient
        = shared_key (32 bytes)

STEP 3: Decrypt
    │
    └──► simple_decrypt()
        ciphertext XOR shared_key
        (with state chaining)
        ↓
    Plaintext = "Hello Bob!"

STEP 4: Display
    │
    └──► Print to console
        "[Sender] Hello Bob!"

STEP 5: Log to File
    │
    └──► [Time] | [Sender] | [N/A]
         | [Hex] | [Details] | [Plaintext]

STEP 6: User Sees
    │
    ▼
"[Sender] Hello Bob!"
```

---

## 5️⃣ Data Structure Layout

### **In-Memory (Arena)**

```
┌────────────────────────────────────────────────────┐
│             MEMORY ARENA (1 MB Total)              │
├────────────────────────────────────────────────────┤
│ [Arena Metadata: 56 bytes]                         │
├────────────────────────────────────────────────────┤
│ [KeyPair #1: Public (32B) + Secret (32B)]         │
├────────────────────────────────────────────────────┤
│ [KeyPair #2: Public (32B) + Secret (32B)]         │
├────────────────────────────────────────────────────┤
│ [Nonce: 16 bytes]                                  │
├────────────────────────────────────────────────────┤
│ [Ciphertext: Variable (msg_len + 16)]             │
├────────────────────────────────────────────────────┤
│ [MAC: 16 bytes]                                    │
├────────────────────────────────────────────────────┤
│ [Free Space: ~1 MB - Used]                        │
└────────────────────────────────────────────────────┘
```

### **Log File (Text)**

```
[2024-01-10 14:30:45] | Server (SENT) | Hello Bob! | 02b27a9201... | 256-bit XOR-Chain | Nonce: d203ada0ed... | MAC: 128-bit | Data Length: 63B | N/A
[2024-01-10 14:30:46] | Client (RECEIVED) | N/A | 02b27a9201... | 256-bit XOR-Chain | Nonce: d203ada0ed... | MAC: 128-bit | Data Length: 63B | Hello Bob!
[2024-01-10 14:30:47] | Client (SENT) | Thanks! | 5f3e1a7c9d... | 256-bit XOR-Chain | Nonce: a1b2c3d4e5... | MAC: 128-bit | Data Length: 47B | N/A
```

---

## 6️⃣ Encryption Algorithm Visualization

### **XOR-Chain Encryption**

```
PLAINTEXT:   H    e    l    l    o    B    o    b    !
ASCII:      [72] [101][108][108][111][66] [111][98] [33]

SHARED KEY:  [k0] [k1] [k2] [k3] [k4] [k5] [k6] [k7] [k0]...

STATE:       0x00
             ▼
             [st + k0] XOR [72]  ──► c1 = 0x45 (state = 0x45)
             [45 + k1] XOR [101] ──► c2 = 0x7A (state = 0x7A)
             [7A + k2] XOR [108] ──► c3 = 0x2E (state = 0x2E)
             ...
             
CIPHERTEXT: [0x45] [0x7A] [0x2E] ... [encrypted bytes]

DECRYPTION (Same algorithm, applied to ciphertext):
CIPHERTEXT: [0x45] [0x7A] [0x2E] ...
STATE:      0x00
            ▼
            [0 + k0] XOR [0x45] ──► 72 = 'H' (state = 0x45)
            [45 + k1] XOR [0x7A] ──► 101 = 'e' (state = 0x7A)
            ...
            
PLAINTEXT:  "Hello Bob!"
```

---

## 7️⃣ Class Interaction Diagram

```
┌─────────────────────────────────────────────────────────────────────┐
│                      CLASS RELATIONSHIPS                            │
└─────────────────────────────────────────────────────────────────────┘

┌────────────────┐
│  SimpleCrypto  │ ◄───┐
│  (static)      │     │
├────────────────┤     │
│ · random_bytes │     │
│ · encrypt      │     ├──────► Used by Message
│ · decrypt      │     │
│ · compute_auth │     │
│ · verify_auth  │     │
└────────────────┘     │

┌────────────────┐
│ CryptoEngine   │ ───►│
│ (init)         │
└────────────────┘

┌─────────────────┐
│   MemArena      │ ◄───┐
│  (memory pool)  │     │
├─────────────────┤     │
│ · push()        │     │
│ · pop()         │     ├──────► Used by
│ · clear()       │     │      - Server
└─────────────────┘     │      - Client
                        │      - KeyPair
                        │      - Message
    
┌─────────────────┐
│    KeyPair      │ ◄───────────► Message
│  · public_key   │
│  · secret_key   │
│  · generate()   │
└─────────────────┘

┌─────────────────┐
│    Message      │ ─────────────► MessageLogger
│  · encrypted    │
│  · nonce        │
│  · mac          │
│  · encrypt()    │
│  · decrypt()    │
└─────────────────┘

┌──────────────────┐
│ MessageLogger    │
│  · log_sent()    │
│  · log_recv()    │
│  · timestamp()   │
└──────────────────┘
        ▲
        │ writes to
        ▼
   logs/messages.txt

┌──────────────────┐         ┌──────────────────┐
│ SecureServer     │◄────┬──►│ SecureClient     │
│ · start()        │     │   │ · connect()      │
│ · run()          │     │   │ · run()          │
│ · encrypt_msg()  │     │   │ · encrypt_msg()  │
│ · decrypt_msg()  │     │   │ · decrypt_msg()  │
└──────────────────┘     │   └──────────────────┘
        ▲                │
        └────TCP/IP ─────┘
          (port 9001)
```

---

## 8️⃣ Timeline: Message Exchange

```
TIME    SERVER                          CLIENT
────────────────────────────────────────────────────────────
T00     Start listening on :9001
        │
T01                                     Connect to :9001
        Accept connection
        │
T02     Generate keypair (pk_s, sk_s)  Generate keypair (pk_c, sk_c)
        │
T03     Send pk_s ──────────────────────►
        │
T04                                     Receive pk_s
        │                               Send pk_c ────────────┐
T05     ◄───────────── Receive pk_c    │
        │
T06     Ready!                          Ready!
        │                               │
        │ User types: "Hello"           │
T07     │                               │
        │                               Encrypt: "Hello"
        │                               Send: [nonce|cipher|mac]
T08     ◄────[nonce|cipher|mac] ────────
        │
T09     Verify MAC
        Decrypt: "Hello"
        Display: "[Client] Hello"
        Log entry: RECEIVED
        │
        │ User types: "Hi!"
T10     │
        Encrypt: "Hi!"
        Send: [nonce|cipher|mac]
        │
T11     ───────[nonce|cipher|mac]─────►
        │
T12                                     Verify MAC
                                        Decrypt: "Hi!"
                                        Display: "[Server] Hi!"
                                        Log entry: RECEIVED
        │
T13     [Continue messaging...]
        │
T14     User types 'exit'
        Close socket
        │
T15                                     Close socket
```

---

## 9️⃣ File I/O - Log Structure

```
LOGS FILE: messages.txt
┌──────────────────────────────────────────────────────────┐
│ ════════════════════════════════════════════════════════ │
│ SECURE MESSAGING SYSTEM - CONVERSATION LOG               │
│ ════════════════════════════════════════════════════════ │
│                                                          │
│ Format: [DateTime] | [Sender] | [Original] | [Encrypted]│
│ | [Details] | [Decrypted]                              │
│ ════════════════════════════════════════════════════════ │
│                                                          │
│ COLUMN 1        COLUMN 2        COLUMN 3               │
│ DateTime        Sender          Original                │
│ 2024-01-10      Server (SENT)   "Hello!"               │
│ 14:30:45        Client (REC)    "N/A"                  │
│                                                          │
│ COLUMN 4        COLUMN 5        COLUMN 6               │
│ Encrypted       Details         Decrypted               │
│ 02b27a92...     256-bit XOR...  "N/A"                  │
│ 02b27a92...     256-bit XOR...  "Hello!"               │
│                                                          │
│ [continuing entries...]                                │
└──────────────────────────────────────────────────────────┘
```

---

## 🔟 Process Flow: Complete Cycle

```
┌────────────────────────────────────────────────────────────┐
│             COMPLETE MESSAGE EXCHANGE CYCLE                │
└────────────────────────────────────────────────────────────┘

USER INTERACTION:
  "Type message and press Enter"
         │
         ▼
  Message Input Buffer
         │
         ├──────────────────────────────────────┐
         │                                      │
    [SENDER]                              [RECEIVER]
         │                                      │
         ▼                                      │
  Create Message                                │
    - Generate nonce                            │
    - Derive key                                │
    - Encrypt plaintext                         │
    - Compute MAC                               │
         │                                      │
         ▼                                      │
  Package Message                               │
    [Nonce|Cipher|MAC]                          │
         │                                      │
         ▼                                      │
  Send over Socket ───────────────────────────►│
    write() to TCP                              │
         │                                      │
                                                ▼
                                          Receive Packet
                                            read() TCP
                                                │
                                                ▼
                                          Verify MAC
                                          ✓ Pass ────┐
                                          ✗ Fail ──► Discard
                                                │
                                                ▼
                                          Decrypt Message
                                            - Derive key
                                            - Decrypt text
                                                │
                                                ▼
                                          Display Message
                                            Console output
                                                │
                                                ▼
                                          Log Entry
                                            Append to file
                                                │
         ◄──────────────────────────────────────┘
         │
    [Both write to same log file]
         │
         ▼
    logs/messages.txt
    (Permanent record)
```

---

## Reference: XOR Encryption

- https://github.com/vineetkia/xor-encryption-c
- https://github.com/mightbesimon/XOR-file-encryption
- https://github.com/m4st3r3k/xor_crypt

---

## Summary

This visual guide shows:
1. **System Architecture** - How server and client interact
2. **Connection Flow** - Key exchange handshake
3. **Encryption Process** - Step-by-step message encryption
4. **Decryption Process** - Step-by-step message decryption
5. **Data Structures** - Memory and file layouts
6. **Algorithm Details** - XOR-Chain cipher visualization
7. **Class Relationships** - How components work together
8. **Timeline** - Message exchange sequence
9. **File Format** - Log file structure
10. **Complete Cycle** - Full message exchange workflow

All diagrams show the complete flow of data through the system! 🚀
